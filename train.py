...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  net_g.train()
  net_d.train()
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid, eid) in enumerate(train_loader):
    x, x_lengths = x.cuda(rank, non_blocking=True), x_lengths.cuda(rank, non_blocking=True)
    spec, spec_lengths = spec.cuda(rank, non_blocking=True), spec_lengths.cuda(rank, non_blocking=True)
    y, y_lengths = y.cuda(rank, non_blocking=True), y_lengths.cuda(rank, non_blocking=True)
    sid, eid = sid.cuda(rank, non_blocking=True), eid.cuda(rank, non_blocking=True)
    with autocast(enabled=hps.train.fp16_run):
      y_hat, l_length, attn, ids_slice, x_mask, z_mask,\
      (z, z_p, m_p, logs_p, m_q, logs_q, e, log_q_e, q_e) = net_g(x, x_lengths, spec, spec_lengths, sid, temp)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_dur + loss_kl  + loss_sup + loss_unsup
    optim_g.zero_grad()
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_dur, loss_kl, loss_sup, loss_unsup]
        logger.info('Train Epoch: {} [{:.0f}%]'.format(
          epoch,
          100. * batch_idx / len(train_loader)))
        logger.info(f"loss/d/total:{loss_disc_all}, loss/g/total:{loss_gen_all}, lr:{lr}, grad_norm_d:{grad_norm_d}, grad_norm_g:{grad_norm_g}, loss/g/fm:{loss_fm},loss/g/mel:{loss_mel}, loss/g/dur:{loss_dur}, loss/g/kl:{loss_kl}, loss/g/sup:{loss_sup}, loss/g/unsup:{loss_unsup}")
        scalar_dict = {"loss/g/total": loss_gen_all, "loss/d/total": loss_disc_all, "learning_rate": lr, "grad_norm_d": grad_norm_d, "grad_norm_g": grad_norm_g}
        scalar_dict.update({"loss/g/fm": loss_fm, "loss/g/mel": loss_mel, "loss/g/dur": loss_dur, "loss/g/kl": loss_kl, "loss/g/sup":loss_sup, "loss/g/unsup":loss_unsup})
...
def evaluate(hps, generator, eval_loader, writer_eval):
    generator.eval()
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid, eid) in enumerate(eval_loader):
        x, x_lengths = x.cuda(0), x_lengths.cuda(0)
        spec, spec_lengths = spec.cuda(0), spec_lengths.cuda(0)
        y, y_lengths = y.cuda(0), y_lengths.cuda(0)
        sid, eid = sid.cuda(0), eid.cuda(0)
        ...
        eid = eid[:1]
        break
      y_hat, attn, mask, *_ = generator.module.infer(x, x_lengths, spec, spec_lengths, sid, max_len=1000)
